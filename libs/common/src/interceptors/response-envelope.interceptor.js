...
let ResponseEnvelopeInterceptor = class ResponseEnvelopeInterceptor {
    ...
    intercept(context, next) {
        ...
        return next.handle().pipe((0, operators_1.tap)(() => {
            ...
            this.logger.log(`Request processed: ${request.method} ${request.url} - ${statusCode} - ${processingTime}ms`);
        }), (0, operators_1.map)((data) => {
            this.logger.debug(`Response envelope created for: ${request.method} ${request.url} - Data type: ${typeof data}`);
            return {
                success: true,
                statusCode,
                message: 'Success',
